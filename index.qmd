...
```{r}
gebco_tif <- glue("{dir_raw}/depth/gebco_2023_n31.0_s23.0_w-98.0_e-81.0.tif")

r_gebco <- rast(gebco_tif) * -1
r_lon <- init(r_gebco, "x")
r_lat <- init(r_gebco, "y")
r_rng <- r_gebco >= 100 &
  r_gebco <= 400 & 
  r_lon > -87.5 &
  r_lat > 27.5
ply_rng <- as.polygons(r_rng) |> 
  st_as_sf() |> 
  rename(value = 1) |> 
  filter(value == 1) |> 
  st_cast("POLYGON") |> 
...
names(r_gebco_study) <- "depth_m"
plet(
  r_gebco_study, col="YlGnBu",
  tiles=providers$Stamen.TonerLite) |> 
  addPolygons(
    data = ply_bia, color="pink", 
    group = "ply_bia") |> 
  addPolygons(
    data = ply_rng, color="purple",
    group = "ply_rng") |> 
  addLegend(
    colors = c("pink", "purple"),
    labels = c("BIA (LeBrecque, 2016)", "Whale Area (BioOp)"),
    group  = c("ply_bia", "ply_rng"))
```
