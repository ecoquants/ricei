...
if (!all(file.exists(c(
  study_gpkg, ships_csv, whales_csv)))){
  ...
  if (!file.exists(study_geo)){
    ...
    us_eez <- mr_features_get(
      ...
      featureID = "eez.281")  |>
      geojson_sf() |> 
      st_transform(prj) # mapView(us_eez)
    
    grid_ships_u <- st_union(grid_ships)
    hex_whales_u <- st_union(hex_whales)
    
    grid_ships_u_buf100km <- grid_ships_u |> 
      st_buffer(100000)
    # mapView(grid_ships_u)
    # mapView(grid_ships_u_buf100km)
    ...
    # write geojson
    write_sf(ply_study, study_gpkg, delete_dsn = T)
  }
